...
Wait a second! We know that Lasso  puts bias in the  coefficients on purpose in order to obtain the lowest out-of-sample MSE. No wonder that the estimator is biased. What if we run a proper OLS but with the variables selected by Lasso? This approach is called *post-Lasso estimator*.

> Post-Lasso estimator is an OLS estimator, but the  variables that are included in that OLS have been selected earlier using Lasso. Post-Lasso estimator was introduced by Belloni and Chernozhukov (2013).
...
So why did post-Lasso procedure lied to us so blatantly?
...
### Run naive post-Lasso estimator one-hundred times
...
In order to understand the Double Selection or Partialling-out via Post-Lasso approaches for inference, we first need to take another look at the OLS, and in particular at its partialling-out properties.
...
## Double Selection or Partialling-out via Post-Lasso procedure
...
### Partialling-out via Post-Lasso (cross-validation) Step-by-Step
...
> For conciseness purposes, I will call the partialling-out via Post-Lasso estimator as post-Double-Selection estimator. Although the two are similar in nature, there is a slight difference between the two. I explain the difference in the video lecture. 
